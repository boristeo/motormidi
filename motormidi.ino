...
constexpr motor* channels[3] = {&x, &y, &z};  // For easy access
constexpr size_t num_channels = sizeof(channels) / sizeof(channels[0]);
uint16_t* interrupt_counts[num_channels] = {&TCNT3, &TCNT4, &TCNT5};
uint16_t* interrupt_limits[num_channels] = {&OCR3A, &OCR4A, &OCR5A};
...
struct __attribute__((packed)) minimidi {
  uint16_t dtime; uint8_t note; uint8_t channel; };
...
void setup() {
  ...
  cli();
  TCCR3A = 0;
  TCCR3B = (1 << WGM12) | (1 << CS11);  // CTC mode, prescale 8x -> count = .5 us
  TIMSK3 = (1 << OCIE3A);
  TCCR4A = 0;
...
void loop() {
  for (auto i = 0; i < num_midis; ++i) {
    ...
    *(uint32_t*)(&m) = pgm_read_dword(&midis[i]);
    ...
    delay(time);
    *interrupt_counts[channel] = 0;
    *interrupt_limits[channel] = note_period * 2;
